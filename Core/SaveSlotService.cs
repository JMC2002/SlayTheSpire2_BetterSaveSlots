...
public readonly record struct SaveSlotImportResult(int Copied, int SkippedEmpty, int SkippedByUser, int Failed);
...
public static class SaveSlotService
{
    ...
    public static async Task<SaveSlotImportResult> ImportNormalProfilesToModdedAsync(
        int slotCount,
        Func<int, Task<bool>> confirmOverwriteAsync)
    {
        ArgumentNullException.ThrowIfNull(confirmOverwriteAsync);

        int copied = 0;
        int skippedEmpty = 0;
        int skippedByUser = 0;
        int failed = 0;
        int maxProfileId = Math.Clamp(slotCount, BetterSaveSlotsSettings.VanillaSlotCount, BetterSaveSlotsSettings.MaxSlotCount);
        ISaveStore store = GetSaveStore();
        ...
        for (int profileId = 1; profileId <= maxProfileId; profileId++)
        {
            try
            {
                ...
                bool targetHasSave = ProfileHasSave(profileId, SaveSlotMode.Modded);
                if (targetHasSave && !await confirmOverwriteAsync(profileId))
                {
                    skippedByUser++;
                    continue;
                }

                await CopyProfileAsync(
                    profileId,
                    profileId,
                    SaveSlotMode.Normal,
                    SaveSlotMode.Modded,
            ...
            {
                failed++;
                ModLogger.Error($"导入 {profileId} 号普通存档失败。", ex);
            }
        }

        return new SaveSlotImportResult(copied, skippedEmpty, skippedByUser, failed);
    }
